...
readSile :: ReaderOptions -- ^ Reader options
          -> String        -- ^ String to parse (assumes @'\n'@ line endings)
          -> Either PandocError Pandoc
readSile opts = readWith parseSile def{ stateOptions = opts }
...
ignoreInlines name = (name, doraw <|> (mempty <$ optargs))
  where optargs = skipopts *> skipMany (try $ optional sp *> braced)
        contseq = '\\':name
        doraw = (rawInline "sile" . (contseq ++) . snd) <$>
...
ignoreBlocks name = (name, doraw <|> (mempty <$ optargs))
  where optargs = skipopts *> skipMany (try $ optional sp *> braced)
        contseq = '\\':name
        doraw = (rawBlock "sile" . (contseq ++) . snd) <$>
                 (getOption readerParseRaw >>= guard >> withRaw optargs)
...
blockCommands = M.fromList $
  ...
  -- in which case they will appear as raw sile blocks
...
inlineCommands = M.fromList $
  [ ("emphasis", extractSpaces emph <$> tok)
...
rawSileBlock :: LP String
rawSileBlock = snd <$> try (withRaw (environment <|> blockCommand))

rawSileInline :: LP Inline
rawSileInline = do
  raw <- (snd <$> withRaw inlineCommand) <|> (snd <$> withRaw blockCommand)
  RawInline "sile" <$> applyMacros' raw
...
citation name mode multi = do
  ...
  return $ cite c (rawInline "sile" $ "\\" ++ name ++ raw)
