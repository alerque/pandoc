...
Tests for the SILE reader.
-}
module Tests.Readers.SILE (tests) where

import Prelude
import Data.Text (Text)
import qualified Data.Text as T
import qualified Text.Pandoc.UTF8 as UTF8
import Text.Pandoc.Readers.SILE (tokenize, untokenize)
import Test.Tasty
...
sile :: Text -> Pandoc
sile = purely $ readSILE def{
                   readerExtensions = getDefaultExtensions "sile" }
...
tests = [ testGroup "tokenization"
          [ testCase "tokenizer round trip on test case" $ do
                 orig <- T.pack <$> UTF8.readFile "../test/sile-reader.sile"
                 let new = untokenize $ tokenize "../test/sile-reader.sile"
                             orig
        ...
        , testGroup "bibsile roman numerals"
          [ "upper" =:
            "number \\RN{12}" =?>
...
bibsileCitations :: TestTree
bibsileCitations = testGroup "bibsile"
